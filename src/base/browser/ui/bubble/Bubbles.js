...
export function Bubbles(container, self, options) {
  var interactionsSave, interactionsSaveCommit, dots, questionsHTML, containerHeight, wait, minTypingWait, scrollDifference, scrollHop;
  // options
  options = typeof options !== 'undefined' ? options : {};
  var animationTime = (this.animationTime = options.animationTime || 200); // how long it takes to animate chat bubble, also set in CSS
  var typeSpeed = (this.typeSpeed = options.typeSpeed || 5); // delay per character, to simulate the machine "typing"
  var widerBy = (this.widerBy = options.widerBy || 2); // add a little extra width to bubbles to make sure they don't break
  var sidePadding = (this.sidePadding = options.sidePadding || 6); // padding on both sides of chat bubbles
  var recallInteractions = (this.recallInteractions = options.recallInteractions || 0); // number of interactions to be remembered and brought back upon restart
  var inputCallbackFn = (this.inputCallbackFn = options.inputCallbackFn || false); // should we display an input field?
  var responseCallbackFn = (this.responseCallbackFn = options.responseCallbackFn || false); // is there a callback function for when a user clicks on a bubble button
  ...
  var interactionsSaveCommit = function () {
    if (!localStorageAvailable) return;
  ...
  this.answer = function (key, content) {
    ...
    _convo[key] !== undefined ? (this.reply(_convo[key]), (standingAnswer = key)) : typeof responseCallbackFn === 'function' ? responseCallbackFn({ input: key, convo: _convo, standingAnswer: standingAnswer }, key) : func(key, content);
